...
namespace CafeShopManagementSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=""C:\Users\Lenovo\Desktop\Stuff\BTQM\03_NguyenHoaiVietChuong_2431101169404_BòTơQuánMộc\Cơ sở dữ liệu\BTQM.mdf"";Integrated Security=True;Connect Timeout=30"))
            {
                connect.Open();
            ...
            if (selectedValue != null)
            {
                ...
                try
                {
                    using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=""C:\Users\Lenovo\Desktop\Stuff\BTQM\03_NguyenHoaiVietChuong_2431101169404_BòTơQuánMộc\Cơ sở dữ liệu\BTQM.mdf"";Integrated Security=True;Connect Timeout=30"))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(selectData, connect))
                        {
                            cmd.Parameters.AddWithValue("@status", "Còn");
        ...
        private void cashierOrderForm_amount_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    MessageBox.Show("Không phù hợp", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cashierOrderForm_amount.Text = "";
                    cashierOrderForm_change.Text = "";
                }
            }
        ...
        private void cashierOrderForm_payBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Bạn chắc chắn muốn thanh toán?", "Confirmation Message"
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "BÒ TƠ QUÁN MỘC";
            ...
            e.Graphics.DrawString("Tổng thành tiền: \tVND" + totalPrice + "\nKhoản tiền: \tVND"
                + cashierOrderForm_amount.Text + "\n\t\t------------\nSố dư: \tVND" + cashierOrderForm_change.Text, labelFont, Brushes.Black, labelX, y);
        ...
        private void cashierOrderForm_removeBtn_Click(object sender, EventArgs e)
        {
            if (getOrderID == 0)
            {
                MessageBox.Show("Xin hãy chọn mặt hàng.", "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn loại bỏ đơn hàng với ID: " + getOrderID + "?", "Confirmation Message"
                , MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            using(SqlCommand cmd = new SqlCommand(deleteData, connect))
                            {
                                ...
                                MessageBox.Show("Loại bỏ thành công!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        catch(Exception ex)
                        {
